function F = ddeVolt(k, v, onevar)
% THIS FUNCTION WAS TAKEN FROM THE CHEBFUN PROJECT AND SLIGHTLY MODIFIED TO WORK WITH DDEs
%
...
function F = volt_col(k, v, onevar)
    ...
    v.func = join(chebfun(0,[-1,0],'splitting','on'),v.func);
    v.func = newDomain(v.func,[0,3]);
    v.func = restrict(v.func,[0,2]);
